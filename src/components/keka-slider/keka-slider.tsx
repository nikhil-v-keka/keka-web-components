  ...
  onRangeInput(event: InputEvent){
    ...
    if(rightValue - leftValue < this.minGap){
      if((event.target as HTMLInputElement).className.includes('range-min'))
          (rangeInput[0] as HTMLInputElement).value = rightValue - this.minGap +'';
      else
          (rangeInput[1] as HTMLInputElement).value = leftValue + this.minGap +'';
    }
    else{
      ...
      <Host>
        <div class="wrapper">
              ...
              <div class="slider-range-input">
                  ...
                  <input type="range" id="rangeMax" class="range-input range-max" min={this.minValue} max={this.maxValue} value={this.rightValue} onInput={this.onRangeInput.bind(this)}/>
              </div>
